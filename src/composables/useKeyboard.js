...
export default function useKeyboard (props, context, dep)
{
  ...
  const handleAddTag = (e) => {
    ...
    if (e.keyCode === 186 && addTagOn.value.indexOf('semi-colon') !== -1 && createTag.value) {
      search.value = search.value.replaceAll(';', '')
    ...
    if (e.keyCode === 188 && addTagOn.value.indexOf('comma') !== -1 && createTag.value) {
      search.value = search.value.replaceAll(',', '')
